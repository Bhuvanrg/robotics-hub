...
-- Buckets are created via Dashboard or API. Ensure a bucket named 'avatars' exists.

-- Enable RLS on storage.objects if not already (Supabase defaults to enabled)
...
-- Allow authenticated users to upload/update/delete only under their own folder `${auth.uid()}/...`
DO $$ BEGIN
