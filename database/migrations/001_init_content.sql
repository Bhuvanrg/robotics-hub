-- 001_init_content.sql
-- Feed & Forums initial schema for Supabase
-- Safe / idempotent patterns: use IF NOT EXISTS; separate constraints.
...
create table if not exists public.thread_replies (
  ...
  author_id   uuid references auth.users(id) on delete cascade,
  body        text not null,
  is_answer   boolean not null default false, -- derived if using answered_reply_id
  created_at  timestamptz not null default now(),
  updated_at  timestamptz not null default now()
);

-- Now that thread_replies exists, ensure answered_reply_id FK is valid (if created before) -- handled above by deferred constraint.
...
-- TRIGGERS: maintain reply_count & last_activity_at
...
-- TRIGGER: enforce single accepted answer consistency (optional sync of is_answer)
create or replace function public.sync_answer_flags()
returns trigger language plpgsql as $$
begin
  -- When thread answered_reply_id changes, set is_answer flags
...
-- OPTIONAL: moderation roles (placeholder) -> you can later add a check on auth.jwt() claims
-- Example: allow delete by a 'moderator' claim
-- create policy forum_threads_moderator_delete on public.forum_threads for delete using ( coalesce((auth.jwt() ->> 'role') = 'moderator', false) );

-- Helpful lower-case tag normalization (function + trigger)
