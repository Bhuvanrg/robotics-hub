-- mirrored from database/migrations/004_engagement.sql
alter table public.user_profiles add column if not exists followed_tags text[] not null default '{}';
...
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='saved_items' AND policyname='saved_items_rw') THEN
